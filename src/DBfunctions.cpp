...
bool insert_points_as_nodes_normal_graph(PGconn *conn, const std::vector<std::vector<std::string>> &data)
{
    ...
    // var declarations
    std::string start = "select * from cypher('normal_graph',$$ UNWIND [";

    std::string end = "] as row CREATE (s:Point {id: row.point}) $$) as (n agtype);";
    ...
    for (size_t i = 0; i < data.size(); i += BATCH_SIZE)
    {
        ...
        for (size_t j = i; j < i + BATCH_SIZE && j < data.size(); ++j)
        {
            ...
            if (seen.find(data[j][1]) == seen.end() && seen.find(data[j][2]) == seen.end())
            {
                middle += "{point:" + data[j][1] + "}, {point:" + data[j][2] + "},";
                seen.insert(data[j][1]);
                seen.insert(data[j][2]);
            }
            else if (seen.find(data[j][1]) == seen.end() && seen.find(data[j][2]) != seen.end())
            {
                middle += "{point:" + data[j][1] + "},";
                seen.insert(data[j][1]);

            }
            else if (seen.find(data[j][1]) != seen.end() && seen.find(data[j][2]) == seen.end())
            {
                middle += "{point:" + data[j][2] + "},";
                seen.insert(data[j][2]);
            }
        }
        if (!middle.empty())
        ...
        if (PQresultStatus(res) != PGRES_TUPLES_OK)
        {
            std::cerr << "insert_points_as_nodes() failed: " << PQerrorMessage(conn) << std::endl;
            PQclear(res);
            return false;
        }
        PQclear(res);
    ...
    std::cout << "insert_points_as_nodes_normal_graph() took: " << time_elapsed.count() << " seconds" << std::endl;
...
bool add_segments_as_edges_to_normal_graph(PGconn *conn, std::vector<std::vector<std::string>> &data)
{
    ...
    std::string start = "SELECT * FROM cypher('normal_graph',$$UNWIND [ ";
    std::string end = "] AS edge "
                      "MATCH (a:Point {id: edge.start_point}), (b:Point {id: edge.end_point }) "
                      "CREATE (a)-[:CONNECTED_TO {id: edge.id,name: edge.name, direction: edge.direction , category: edge.category}]->(b) "
                      "$$) AS (e agtype);";
    ...
    for (size_t i = 0; i < data.size(); i += BATCH_SIZE)
    {
        ...
        for (size_t j = i; j < i + BATCH_SIZE && j < data.size(); ++j)
        {
           middle += "{id: " + data[j][0] +                  // int
          ", start_point: " + data[j][1] +        // int
          ", end_point: " + data[j][2] +          // int
          ", category: \'" + data[j][3] + "\'" +   // string
          ", direction: \'" + data[j][4] +          // int (if it is)
          "\', name: \'" + data[j][5] + "\'},";
        }
        ...
        if (PQresultStatus(res) != PGRES_TUPLES_OK)
        {
            std::cerr << "add_segments_as_edges_to_normal_graph() failed: " << PQerrorMessage(conn) << std::endl;
            PQclear(res);
            return false;
        }
        PQclear(res);
    ...
    auto finish2{std::chrono::steady_clock::now()};
    std::chrono::duration<double> time_elapsed2{finish2 - start_timer};
    std::cout << "add_segments_as_edges_to_normal_graph() took: " << time_elapsed2.count() << " seconds" << std::endl;
